...
This collection packages the new reusable methods exercised in the shared "Completion status summary" session. The source session was still active when captured and ended at a fresh owner OAuth consent checkpoint.
...
| [oauth-state-recovery](oauth-state-recovery/SKILL.md) | Recover expired or cached OAuth transactions without weakening state validation |
...
Five standard-library Python helpers validate:
